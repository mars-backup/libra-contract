...
describe("Swap Deployer", () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      TOKENS.push(BUSD, USDC, USDT, SUSD)
      ...
      const swapCloneAddress = await swapDeployer.callStatic.deploy(
        swap.address,
        [BUSD.address, USDC.address, USDT.address, SUSD.address],
        [18, 18, 18, 18],
        LP_TOKEN_NAME,
        LP_TOKEN_SYMBOL,
        INITIAL_A_VALUE,
      ...
      await asyncForEach([owner, user1, user2, attacker], async (signer) => {
        await BUSD.connect(signer).approve(swapClone.address, MAX_UINT256)
      ...
      expect(await swapClone.getTokenBalance(1)).to.be.eq(String(50e18))
      expect(await swapClone.getTokenBalance(2)).to.be.eq(String(50e18))
  ...
  describe("removeLiquidity", () => {
    it("Remove Liquidity succeeds", async () => {
      ...
      await swapToken
        .connect(user1)
        .approve(swapClone.address, "999354692964991477")
  ...
  describe("Check for timestamp manipulations", () => {
    ...
    it("Check for maximum differences in A and virtual price when increasing", async () => {
      ...
      expect(await swapClone.getVirtualPrice()).to.be.eq("1000166123967579214")
      ...
      expect(await swapClone.getVirtualPrice()).to.be.eq("1000168048353737315")
    ...
    it("Check for maximum differences in A and virtual price when decreasing", async () => {
      ...
      expect(await swapClone.getVirtualPrice()).to.be.eq("1000165482010262682")
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            ...
            expect(await swapClone.getTokenBalance(1)).to.be.eq(String(66e18))
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            ...
            expect(await swapClone.getTokenBalance(1)).to.be.eq(String(75e18))
            expect(await swapClone.getTokenBalance(3)).to.be.eq(
