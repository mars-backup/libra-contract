...
describe("Meta-Swap", async () => {
  ...
  describe("removeLiquidity", () => {
    ...
    it.skip("Succeeds even when contract is paused", async () => {
  ...
  describe("removeLiquidityOneToken", () => {
    ...
    it.skip("Reverts with 'Withdraw exceeds available'", async () => {
      // User 1 adds liquidity
      await metaSwap
        .connect(user1)
        .addLiquidity([String(2e18), String(1e16)], 0, MAX_UINT256)
  ...
  describe("swapUnderlying", () => {
    ...
    describe("Succeeds with expected swap amounts", () => {
      ...
      it("From 18 decimal token (base) to 18 decimal token (meta)", async () => {
        // User 1 calculates how much token to receive
        const calculatedSwapReturn = await metaSwap.calculateSwapUnderlying(
          1,
          0,
        ...
        const [tokenFromBalanceBefore, tokenToBalanceBefore] =
          await getUserTokenBalances(user1, [busd, susd])

        // User 1 successfully initiates swap
        await metaSwap
          .connect(user1)
          .swapUnderlying(1, 0, String(1e17), minReturnWithNegativeSlippage, MAX_UINT256)
  ...
  describe("withdrawAdminFees", () => {
    it("Reverts when called by non-owners", async () => {
      await expect(metaSwap.connect(user1).withdrawAdminFees(ownerAddress)).to.be.reverted
      await expect(metaSwap.connect(user2).withdrawAdminFees(ownerAddress)).to.be.reverted
    })
    ...
    it("Succeeds when there are no fees withdrawn", async () => {
      ...
      await metaSwap.withdrawAdminFees(ownerAddress)
    ...
    it("Succeeds with expected amount of fees withdrawn (swapUnderlying)", async () => {
      ...
      expect(await metaSwap.getAdminBalance(0)).to.eq(String(1001774283003))
      expect(await metaSwap.getAdminBalance(1)).to.eq(String(998024139765))
      ...
      expect(firstTokenAfter.sub(firstTokenBefore)).to.eq(String(1001774283003))
      expect(secondTokenAfter.sub(secondTokenBefore)).to.eq(
        String(998024139765),
      )
    })
