...
describe("Meta-Swap Deposit Contract", async () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      await asyncForEach(
        ...
        async (address) => {
          await busd.mint(address, BigNumber.from(10).pow(18).mul(100000))
          await usdc.mint(address, BigNumber.from(10).pow(18).mul(100000))
          await usdt.mint(address, BigNumber.from(10).pow(18).mul(100000))
      ...
      await asyncForEach([owner, user1, user2], async (signer) => {
        ...
        await busd.connect(signer).approve(metaSwap.address, MAX_UINT256)
        ...
        await busd.connect(signer).approve(baseSwap.address, MAX_UINT256)
        ...
        await baseSwap
          .connect(signer)
          .addLiquidity(
            [String(1e20), String(1e20), String(1e20)],
            0,
            MAX_UINT256,
          )
      })
      ...
      allTokens = [susd, busd, usdc, usdt]

      // Approve token transfers to MetaSwapDeposit
      await asyncForEach([owner, user1, user2], async (signer) => {
        await asyncForEach(
          [susd, busd, usdc, usdt, metaLPToken],
          async (token) => {
            await token
              .connect(signer)
  ...
  describe("getToken", () => {
    it("Returns correct token addresses", async () => {
      ...
      expect(await metaSwapDeposit.getToken(1)).to.be.eq(busd.address)
  ...
  describe("swap", () => {
    it("From 18 decimal token (meta) to 18 decimal token (base)", async () => {
      ...
      const [tokenFromBalanceBefore, tokenToBalanceBefore] =
        await getUserTokenBalances(user1, [susd, busd])
    ...
    it.skip("From 6 decimal token (base) to 18 decimal token (meta)", async () => {
      // User 1 calculates how much token to receive
      const calculatedSwapReturn = await metaSwapDeposit.calculateSwap(
        2,
        0,
    ...
    it.skip("From 18 decimal token (meta) to 6 decimal token (base)", async () => {
      // User 1 calculates how much token to receive
      const calculatedSwapReturn = await metaSwapDeposit.calculateSwap(
        0,
        2,
    ...
    it.skip("From 18 decimal token (base) to 6 decimal token (base)", async () => {
      // User 1 calculates how much token to receive
      const calculatedSwapReturn = await metaSwapDeposit.calculateSwap(
        1,
        3,
      ...
      const [tokenFromBalanceBefore, tokenToBalanceBefore] =
        await getUserTokenBalances(user1, [busd, usdt])
  ...
  describe("addLiquidity", () => {
    ...
    it("Succeeds when depositing imbalanced amounts", async () => {
      ...
      expect(balanceAfter.sub(balanceBefore)).to.eq(
        String("2999246305541860228"),
      )
      expect(returnValue).to.eq("2999246305541860228")
    })
    ...
    it("Succeeds when depositing single token (base swap level)", async () => {
      ...
      expect(balanceAfter.sub(balanceBefore)).to.eq(String("999304"))
    })
  })
  ...
  describe("removeLiquidity", () => {
    ...
    it("Succeeds with expected minAmounts", async () => {
      ...
      expect(minAmounts[0]).to.eq("504905420653606879")
      expect(minAmounts[1]).to.eq("165399846564980642")
      expect(minAmounts[2]).to.eq("164850345413601969")
      expect(minAmounts[3]).to.eq("164850345413601969")
      ...
      expect(returnValues[0]).to.eq("504905420653606879")
      expect(balancesAfter[1].sub(balancesBefore[1])).to.eq(
        "165399846564980642",
      )
      expect(returnValues[1]).to.eq("165399846564980642")
      expect(balancesAfter[2].sub(balancesBefore[2])).to.eq("164850345413601969")
      expect(returnValues[2]).to.eq("164850345413601969")
      expect(balancesAfter[3].sub(balancesBefore[3])).to.eq("164850345413601969")
      expect(returnValues[3]).to.eq("164850345413601969")
    })
  })
  ...
  describe("removeLiquidityOneToken", () => {
    ...
    it("Succeeds when withdrawing via a meta level token", async () => {
      ...
      expect(balanceAfter.sub(balanceBefore)).to.eq("999653672246293002")
      expect(returnValue).to.eq("999653672246293002")
    })
    ...
    it("Succeeds when withdrawing via a base level token", async () => {
      ...
      // Check the return value matches the amount withdrawn
      expect(balanceAfter.sub(balanceBefore)).to.eq("999047026786985747")
      expect(returnValue).to.eq("999047026786985747")
    })
  })
  ...
  describe("removeLiquidityImbalance", () => {
    ...
    it("Succeeds when only withdrawing meta-level tokens", async () => {
      ...
      const maxBurnAmount = (
        ...
        .mul(1001)
        .div(1000)
      expect(maxBurnAmount).to.eq("1000850974163953096")

      // Balances before the call
      const tokens = [susd, busd, usdc, usdt, metaLPToken]
      ...
      expect(returnValues).to.eq("1000346218956410768")
    ...
    it("Succeeds when only withdrawing base-level tokens", async () => {
      const amounts = [String(0), String(1e18), String(1e18), String(0)]
      ...
      const maxBurnAmount = (
        ...
        .mul(1002)
        .div(1000)
      expect(maxBurnAmount).to.eq("2004575550570877866")
      ...
      // The return value matches the amount of meta LP token burned
      expect(returnValues).to.eq("2001784980265401923")
      expect(balancesBefore[4].sub(balancesAfter[4])).to.eq(
        "2001784980265401923",
      )
    ...
    it("Succeeds when withdrawing both meta-level and base-level tokens", async () => {
      const amounts = [String(1e18), String(0), String(1e18), String(1e18)]
      ...
      const maxBurnAmount = (
        ...
        .mul(1002)
        .div(1000)
      expect(maxBurnAmount).to.eq("3006248780431811083")
      ...
      // The return value matches the amount of meta LP token burned
      expect(returnValues).to.eq("3000964412046901584")
      expect(balancesBefore[4].sub(balancesAfter[4])).to.eq(
        "3000964412046901584",
      )
