...
describe("Swap Flashloan", () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      await asyncForEach([owner, user1, user2, attacker], async (signer) => {
        await BUSD.connect(signer).approve(swapFlashLoan.address, MAX_UINT256)
        ...
        await USDm.connect(signer).approve(swapFlashLoan.address, MAX_UINT256)
      })
      ...
      expect(await swapFlashLoan.getTokenBalance(1)).to.be.eq(String(50e18))
      expect(await swapFlashLoan.getTokenBalance(2)).to.be.eq(String(50e18))
  ...
  it("Reverts when the borrower does not have enough to pay back", async () => {
    await expect(
      flashLoanExample.flashLoan(swapFlashLoan.address, USDC.address, String(1e18), []),
  ...
  it("Succeeds when fee is paid off", async () => {
    const flashLoanAmount = BigNumber.from(String(1e18))
    ...
    expect(await swapFlashLoan.getTokenBalance(1)).to.eq("50000000000000000000")
    ...
    expect(await swapFlashLoan.getTokenBalance(1)).to.eq("50005000000000000000")
    expect(await swapFlashLoan.getAdminBalance(1)).to.eq("5000000000000000")
    expect(await USDC.balanceOf(swapFlashLoan.address)).to.eq("50010000000000000000")
    ...
    expect(await swapFlashLoan.getTokenBalance(1)).to.eq("50010000000000000000")
    expect(await swapFlashLoan.getAdminBalance(1)).to.eq("10000000000000000")
    expect(await USDC.balanceOf(swapFlashLoan.address)).to.eq("50020000000000000000")

    // Try to withdraw the protocol fees
    await swapFlashLoan.withdrawAdminFees(ownerAddress)
    expect(await swapFlashLoan.getVirtualPrice()).to.eq("1000049999926479164")
    expect(await swapFlashLoan.getTokenBalance(1)).to.eq("50010000000000000000")
  ...
  describe("setFlashLoanFees", () => {
    ...
    it("Succeeds when protocol fee bps is set to 0", async () => {
      ...
      expect(await swapFlashLoan.getTokenBalance(1)).to.eq("50000800000000000000")
      expect(await swapFlashLoan.getAdminBalance(1)).to.eq("0")
      expect(await USDC.balanceOf(swapFlashLoan.address)).to.eq("50000800000000000000")
    })
    ...
    it("Succeeds when fees are in the valid range", async () => {
      ...
      expect(await swapFlashLoan.getTokenBalance(1)).to.eq("50004950000000000000")
      expect(await swapFlashLoan.getAdminBalance(1)).to.eq("50000000000000")
      expect(await USDC.balanceOf(swapFlashLoan.address)).to.eq("50005000000000000000")
    })
  })
})
