...
describe("Meta-Swap with inflated baseVirtualPrice and 50% admin fees", async () => {
  ...
  const AMOUNT = 1e17
  const INFLATED_VP = "1013335608224782131"
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      for (let index = 0; index < 100; index++) {
        ...
        await baseSwap.connect(user1).swap(1, 2, String(1e20), 0, MAX_UINT256)

        await baseSwap.connect(user1).swap(2, 0, String(1e20), 0, MAX_UINT256)
      }
  ...
  describe("swapUnderlying", () => {
    ...
    it("susd -> dai -> susd: outcome not great not terrible", async () => {
      ...
      const tokenToBalanceAfterFirst = await getUserTokenBalance(user1, busd)
