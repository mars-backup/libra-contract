...
describe("Swap with 4 tokens", () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      USDm = await ethers.getContract("USDm")

      TOKENS.push(BUSD, USDC, USDT, USDm)
      ...
      await asyncForEach(
        ...
        async (address) => {
          ...
          await USDC.mint(address, String(1e20))
          await USDT.mint(address, String(1e20))
          await USDm.mint(address, String(1e20))
        },
      )
      ...
      await swap.initialize(
        ...
        [BUSD.address, USDC.address, USDT.address, USDm.address],
        [18, 18, 18, 18],
        LP_TOKEN_NAME,
        LP_TOKEN_SYMBOL,
        INITIAL_A_VALUE,
      ...
      await asyncForEach([owner, user1, user2, attacker], async (signer) => {
        await BUSD.connect(signer).approve(swap.address, MAX_UINT256)
        await USDC.connect(signer).approve(swap.address, MAX_UINT256)
        await USDT.connect(signer).approve(swap.address, MAX_UINT256)
        await USDm.connect(signer).approve(swap.address, MAX_UINT256)
      })

      // Populate the pool with initial liquidity
      await swap.addLiquidity(
        [String(50e18), String(50e18), String(50e18), String(50e18)],
        0,
        MAX_UINT256,
      )
      ...
      expect(await swap.getTokenBalance(2)).to.be.eq(String(50e18))
  ...
  describe("swap", () => {
    it.skip("Swap works between tokens with different decimals", async () => {
      const calcTokenAmount = await swap
        .connect(user1)
        .calculateSwap(2, 0, String(1e18))
      expect(calcTokenAmount).to.be.eq("998608238366733809")
      const DAIBefore = await getUserTokenBalance(user1, BUSD)
      ...
      const DAIAfter = await getUserTokenBalance(user1, BUSD)
  ...
  describe("removeLiquidity", () => {
    it("Remove Liquidity succeeds", async () => {
      ...
      expect(expectedAmounts[0]).to.be.eq("253568422739809031")
      expect(expectedAmounts[1]).to.be.eq("248596492882165717")
      expect(expectedAmounts[2]).to.be.eq("248596492882165717")
      expect(expectedAmounts[3]).to.be.eq("248596492882165717")

      // Allow burn of swapToken
      await swapToken.connect(user1).approve(swap.address, "999354692964991477")
  ...
  describe("withdrawAdminFees", () => {
    it("Reverts when called by non-owners", async () => {
      await expect(swap.connect(user1).withdrawAdminFees(ownerAddress)).to.be.reverted
      await expect(swap.connect(user2).withdrawAdminFees(ownerAddress)).to.be.reverted
    })
    ...
    it("Succeeds when there are no fees withdrawn", async () => {
      ...
      await swap.withdrawAdminFees(ownerAddress)
    ...
    it("Succeeds with expected amount of fees withdrawn", async () => {
      ...
      await swap.connect(user1).swap(1, 0, String(1e18), 0, MAX_UINT256)

      expect(await swap.getAdminBalance(0)).to.eq(String(10003917594870))
      expect(await swap.getAdminBalance(1)).to.eq(String(9996078462129))
      ...
      expect(balancesAfter[0].sub(balancesBefore[0])).to.eq(
        String(10003917594870),
      )
      expect(balancesAfter[1].sub(balancesBefore[1])).to.eq(String(9996078462129))
    })
    ...
    it("Withdrawing admin fees has no impact on users' withdrawal", async () => {
      ...
      await swap
        .connect(user1)
        .addLiquidity(
          [String(1e18), String(1e18), String(1e18), String(1e18)],
          0,
          MAX_UINT256,
        )
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await swap.connect(user2).swap(1, 0, String(1e18), 0, MAX_UINT256)
      }

      expect(await swap.getAdminBalance(0)).to.eq(String(100038269930403))
      expect(await swap.getAdminBalance(1)).to.eq(String(99961691381600))
      ...
      expect(balancesAfter[0].sub(balancesBefore[0])).to.eq(
        BigNumber.from("1000119152856514633"),
      )

      expect(balancesAfter[1].sub(balancesBefore[1])).to.eq(
        BigNumber.from("1000269158221299577"),
      )
    })
  })
  ...
  describe("Check for timestamp manipulations", () => {
    it("Check for maximum differences in A and virtual price when increasing", async () => {
      ...
      expect(await swap.getVirtualPrice()).to.be.eq("1000166123967579214")
      ...
      expect(await swap.getVirtualPrice()).to.be.eq("1000168048353737315")
    ...
    it("Check for maximum differences in A and virtual price when decreasing", async () => {
      ...
      expect(await swap.getVirtualPrice()).to.be.eq("1000165482010262682")
    ...
    describe("Check for attacks while A is ramping upwards", () => {
      ...
      beforeEach(async () => {
        ...
        expect(initialPoolBalances[1]).to.be.eq(String(50e18))
        expect(initialPoolBalances[2]).to.be.eq(String(50e18))
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 16e6 of USDC to USDm, causing massive imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(16e18), 0, MAX_UINT256)
            const USDmOutput = (await getUserTokenBalance(attacker, USDm)).sub(
              initialAttackerBalances[3],
            )

            // First trade results in 15.87e18 of USDm
            expect(USDmOutput).to.be.eq("15873636661935380627")

            // Pool is imbalanced! Now trades from USDm -> USDC may be profitable in small sizes
            // USDC balance in the pool : 66e6
            // USDm balance in the pool : 34.13e18
            expect(await swap.getTokenBalance(1)).to.be.eq(String(66e18))
            ...
            // Trade USDm to USDC, taking advantage of the imbalance and change of A
            const balanceBefore = await getUserTokenBalance(attacker, USDC)
            await swap.connect(attacker).swap(3, 1, USDmOutput, 0, MAX_UINT256)
            ...
            expect(USDCOutput).to.be.eq("15967909813520201926")
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Trade USDm to USDC, taking advantage of the imbalance and sudden change of A
            const balanceBefore = await getUserTokenBalance(attacker, USDC)
            await swap.connect(attacker).swap(3, 1, USDmOutput, 0, MAX_UINT256)
            const USDCOutput = (await getUserTokenBalance(attacker, USDC)).sub(
              balanceBefore,
            ...
            // If USDCOutput > 16e6, the attacker leaves with more USDC than the start.
            expect(USDCOutput).to.be.eq("15913488774152396495")
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swapping 25e6 of USDC to USDm, resolving imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(25e18), 0, MAX_UINT256)
            const USDmOutput = (await getUserTokenBalance(attacker, USDm)).sub(
              initialAttackerBalances[3],
            )

            // First trade results in 25.14e18 of USDm
            // Because the pool was imbalanced in the beginning, this trade results in more than 25e18 USDm
            expect(USDmOutput).to.be.eq("25140480043410581418")

            // Pool is now almost balanced!
            // USDC balance in the pool : 75.00e6
            // USDm balance in the pool : 74.86e18
            expect(await swap.getTokenBalance(1)).to.be.eq(String(75e18))
            expect(await swap.getTokenBalance(3)).to.be.eq(
            ...
            expect(USDCOutput).to.be.eq("24950174606030619003")
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Swap 25e6 of USDC to USDm, resolving the imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(25e18), 0, MAX_UINT256)
            const USDmOutput = (await getUserTokenBalance(attacker, USDm)).sub(
            ...
            // If USDCOutput > 25e6, the attacker leaves with more USDC than the start.
            expect(USDCOutput).to.be.eq("25031387716783736404")
    ...
    describe("Check for attacks while A is ramping downwards", () => {
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            ...
            // If USDCOutput > 16e6, the attacker leaves with more USDC than the start.
            expect(USDCOutput).to.be.eq("15967995987469274451")
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Swap 16e6 USDC to USDm, causing imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(16e18), 0, MAX_UINT256)
            const USDmOutput = (await getUserTokenBalance(attacker, USDm)).sub(
            ...
            // If USDCOutput > 16e6, the attacker leaves with more USDC than the start.
            expect(USDCOutput).to.be.eq("16073391543876051228")
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 25e6 of USDC to USDm, resolving imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(25e18), 0, MAX_UINT256)
            const USDmOutput = (await getUserTokenBalance(attacker, USDm)).sub(
            ...
            // If USDCOutput > 25e6, the attacker leaves with more USDC than the start.
            expect(USDCOutput).to.be.eq("24950046599888173137")
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // First trade results in 25.14e18 of USDm
            // Because the pool was imbalanced in the beginning, this trade results in more than 1e18 USDm
            expect(USDmOutput).to.be.eq("25140480043410581418")
            ...
            // If USDCOutput > 25e6, the attacker leaves with more USDC than the start.
            expect(USDCOutput).to.be.eq("24794844399004032132")
