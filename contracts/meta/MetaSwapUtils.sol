...
library MetaSwapUtils {
        ...
        )
    {
        ...
        bool isLPToken = tokenIndex == xp.length.sub(1);
        ...
        uint256 dy0 = xp[tokenIndex].sub(v.newY);
        ...
        if (isLPToken) {
            ...
            dy0 = dy0.mul(BASE_VIRTUAL_PRICE_PRECISION).div(baseVirtualPrice);
        }
        dy = dy.div(self.tokenPrecisionMultipliers[tokenIndex]);
        dy0 = dy0.div(self.tokenPrecisionMultipliers[tokenIndex]);

        return (dy, dy0.sub(dy));
    }
    ...
    function calculateSwapUnderlying(
        ...
        if (tokenIndexFrom < v.baseLPTokenIndex) {
            ...
            if (tokenIndexTo < v.baseLPTokenIndex) {
                ...
                v.x = v.x.sub(
                    v.x.mul(v.baseSwap.getSwapFee()).div(2 * FEE_DENOMINATOR)
...

        {
            ...
            v.dy = v.dy.sub(v.dy.mul(self.swapFee).div(FEE_DENOMINATOR));
        }
